...
void ParseRecipients(CWallet* const pwallet, const UniValue& address_amounts, const UniValue& subtract_fee_outputs, std::vector<CRecipient> &recipients) {
    ...
    for (const std::string& address: address_amounts.getKeys()) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid XEP address: ") + address);
        }
        ...
        if (dest.which() == 1 /* PKHash */ || dest.which() == 2 /* ScriptHash */) {
            const int nHeight = std::max(pwallet->chain().getHeight().get_value_or(int(0)) - 100, 0);
            if (nHeight >= 180000) {
        ...
        HELP_REQUIRING_PASSPHRASE,
            ...
            "       \"" + FeeModes("\"\n\"") + "\""},
                ...
                RPCExamples{
                    ...
                    "\nSend 0.1 XEP with a confirmation target of 6 blocks in economical fee estimate mode using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"donation\" \"sean's outpost\" false true 6 economical") +
                    "\nSend 0.1 XEP with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB, subtract fee from amount, BIP125-replaceable, using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"drinks\" \"room77\" true true null \"unset\" null 1.1") +
                    "\nSend 0.2 XEP with a confirmation target of 6 blocks in economical fee estimate mode using named arguments\n"
                    + HelpExampleCli("-named sendtoaddress", "address=\"" + EXAMPLE_ADDRESS[0] + "\" amount=0.2 conf_target=6 estimate_mode=\"economical\"") +
                    "\nSend 0.5 XEP with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
        ...
        HELP_REQUIRING_PASSPHRASE,
            ...
            "       \"" + FeeModes("\"\n\"") + "\""},
                ...
                RPCExamples{
                    ...
                    "\nBurn 0.1 XEP with a confirmation target of 6 blocks in economical fee estimate mode using positional arguments\n"
                    + HelpExampleCli("burncoins", "0.1 \"Hello world!\" false true 6 economical") +
                    "\nBurn 0.1 XEP with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB, subtract fee from amount, BIP125-replaceable, using positional arguments\n"
                    + HelpExampleCli("burncoins", "0.1 \"Hello world!\" true true null \"unset\" null 1.1") +
                    "\nBurn 0.2 XEP with a confirmation target of 6 blocks in economical fee estimate mode using named arguments\n"
                    + HelpExampleCli("-named burncoins", "amount=0.2 conf_target=6 estimate_mode=\"economical\"") +
                    "\nBurn 0.5 XEP with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
    ...
    if (!options.isNull()) {
      if (options.type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress") || options.exists("change_address")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Change address must be a valid xep address");
            }
        ...
        [want_psbt](const RPCHelpMan& self, const JSONRPCRequest& request) mutable -> UniValue
{
    ...
    if (pwallet->IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS) && !want_psbt) {
        if (!pwallet->chain().rpcEnableDeprecated("bumpfee")) {
            throw JSONRPCError(RPC_METHOD_DEPRECATED, "Using bumpfee with wallets that have private keys disabled is deprecated. Use psbtbumpfee instead or restart xepd with -deprecatedrpc=bumpfee. This functionality will be removed in 0.22");
        }
        want_psbt = true;
    }
...
static RPCHelpMan send()
{
    return RPCHelpMan{"send",
            ...
            {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                {
                    ...
                    {"change_address", RPCArg::Type::STR_HEX, /* default */ "pool address", "The xep address to receive the change"},
        ...
        RPCExamples{""
        "\nSend 0.1 XEP with a confirmation target of 6 blocks in economical fee estimate mode\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.1}' 6 economical\n") +
        "Send 0.2 XEP with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB using positional arguments\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" 1.1\n") +
        "Send 0.2 XEP with a fee rate of 1 " + CURRENCY_ATOM + "/vB using the options argument\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" null '{\"fee_rate\": 1}'\n") +
        "Send 0.3 XEP with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
